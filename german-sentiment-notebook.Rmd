...
calculate_column_sums <- function(dtm_forms, contained_alt_forms) {
  selected_columns <- dtm_forms[, contained_alt_forms, drop = FALSE]
  
  if (ncol(selected_columns) > 1) {
    result <- apply(selected_columns, 1, sum)
  } else {
...
apply_sentiment_scores <- function(dtm_forms, 
  ...
  # apply them colwise
  # required_scores <- matrix(required_scores, ncol = 1)
...
The sentiment matrix corresponds to
...
# n_sentences <- 500
sentences <- as.vector(treebank_data$full_sentence[1:200])
